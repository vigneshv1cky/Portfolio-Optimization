...
# =====================================================
SECTOR_STOCKS = {
    # (unchanged; your long mapping kept as-is)
...
# =====================================================
# Period helpers (unified everywhere)
# =====================================================
def _last_close_prior_to(s: pd.Series, ts: pd.Timestamp) -> float:
    """Return last close value <= ts (tz-aware safe)."""
    s = s.dropna()
    if s.empty:
        return np.nan
    # ensure ts has same tz as index
...
def ytd_price_return(s: pd.Series) -> float:
    """Return today vs last close of prior year (matches Yahoo YTD convention)."""
    s = s.dropna()
    if s.empty:
        return np.nan
    year = s.index[-1].year
    tz = getattr(s.index, "tz", None)
    jan1 = pd.Timestamp(year, 1, 1, tz=tz)
    prev_val = _last_close_prior_to(s, jan1)
    if np.isnan(prev_val):
        return np.nan
    return float(s.iloc[-1] / prev_val - 1)


def _ret_trading_days(s: pd.Series, n: int) -> float:
    """Exact n-trading-day return (uses pct_change(n))."""
    s = s.dropna()
    if len(s) <= n:
        return np.nan
    return float(s.pct_change(n).iloc[-1])


def _ret_calendar_months(s: pd.Series, months: int) -> float:
    """
    Calendar-anchored return: today vs last close <= (today - months).
    This matches how Yahoo displays 1M/3M/6M/1Y on quote pages.
    """
    s = s.dropna()
    if s.empty:
        return np.nan
    last_ts = s.index[-1]
    anchor = last_ts - pd.DateOffset(months=months)
    anchor_val = _last_close_prior_to(s, anchor)
    if np.isnan(anchor_val):
        return np.nan
    return float(s.iloc[-1] / anchor_val - 1)


def trailing_return_calendar(s: pd.Series, choice: str) -> float:
    if choice == "1W":
        return _ret_trading_days(s, 5)  # keep 1W as 5 trading days
    if choice == "1M":
        return _ret_calendar_months(s, 1)
    if choice == "3M":
        return _ret_calendar_months(s, 3)
    if choice == "6M":
        return _ret_calendar_months(s, 6)
    if choice == "1Y":
        return _ret_calendar_months(s, 12)
    if choice == "YTD":
        return ytd_price_return(s)
    return np.nan


def trailing_return_trading_days(s: pd.Series, choice: str) -> float:
    td = {"1W": 5, "1M": 21, "3M": 63, "6M": 126, "1Y": 252}
    if choice == "YTD":
        return ytd_price_return(s)
    if choice in td:
        return _ret_trading_days(s, td[choice])
    return np.nan
...
with topbar:
    ...
    USE_CALENDAR_PERIODS = cA.toggle(
        "Match Yahoo calendar periods (1M/3M/6M/1Y)",
        value=True,
        help="On = calendar-anchored (Yahoo-like). Off = fixed trading days (21/63/126/252).",
        key="toggle_calendar",
    )
    USE_ADJUSTED = cB.toggle(
        "Use adjusted closes (dividends/splits)",
        value=True,
        help="Off = price-only. Turn off for closer parity with Yahoo quote tiles.",
        key="toggle_adjusted",
    )


def trailing_return_mode(s: pd.Series, choice: str) -> float:
    return trailing_return_calendar(s, choice) if USE_CALENDAR_PERIODS else trailing_return_trading_days(s, choice)
...
def download_close(tickers, period="2y", interval="1d", adjusted=True):
    """
    Returns a DataFrame of Close prices with columns = tickers.
    Set adjusted=False for price-only (unadjusted Close).
    """
    ...
    # Normalize to Close-only with columns=tickers
...
prices = download_close(tickers, period="2y", interval="1d", adjusted=USE_ADJUSTED)
...
for sector, ticker in SECTOR_ETFS.items():
    ...
    row = {
        "Sector": sector,
        "Ticker": ticker,
        "1W": trailing_return_mode(s, "1W"),
        "1M": trailing_return_mode(s, "1M"),
        "3M": trailing_return_mode(s, "3M"),
        "6M": trailing_return_mode(s, "6M"),
        "YTD": trailing_return_mode(s, "YTD"),
        "1Y": trailing_return_mode(s, "1Y"),
        "Vol (20D→Ann)": vol,
        "RS 3M vs SPY": (
            trailing_return_mode(spy_ratio[ticker].dropna(), "3M") if ticker in spy_ratio.columns else np.nan
        ),
    }
    metrics.append(row)
...
if not carried_sector:
    ...
    if not stock_list:
        ...
        stock_close = download_close(stock_list, period="2y", interval="1d", adjusted=USE_ADJUSTED)
        if stock_close.empty:
            ...
            for tk in stock_close.columns:
                ...
                vol = (
                    s.pct_change(fill_method=None).rolling(20).std().iloc[-1] * np.sqrt(252) if len(s) >= 21 else np.nan
                )

                m1 = trailing_return_mode(s, "1M")
                m3 = trailing_return_mode(s, "3M")
                y1 = trailing_return_mode(s, "1Y")
                ytd = trailing_return_mode(s, "YTD")
...
try:
    ...
    if stock_list:
        # multi-ticker download for performance
        ...
        for t in stock_list:
            try:
                ...
                r = trailing_return_mode(close, ranking_choice)
        ...
        if df_stockperf.empty:
            ...
            # --- Build figure (go.Bar to lock y labels and clean tooltip) ---
            dfp = df_stockperf.dropna(subset=["ReturnPct"]).copy()
            ...
            fig_stocks = go.Figure(
                data=go.Bar(
                    ...
                    hovertemplate="%{y}: %{x:.2f}%<extra></extra>",  # clean tooltip: no color/extra fields
                )
            )
            ...
            with row1[1]:
                ...
                # Pretty signal counts (no `{}` dict)
                if not stock_df.empty and "Signal" in stock_df.columns:
                    counts = stock_df["Signal"].value_counts()
                    if counts.empty:
                        st.caption("Signal counts — none")
                    else:
                        st.caption("Signal counts")
                        st.table(counts.rename("Count").to_frame())
                st.markdown("</div>", unsafe_allow_html=True)
...
if df.empty:
    ...
    try:
        ...
        data_raw = yf.download(selected_stock, period="1y", interval="1d", auto_adjust=USE_ADJUSTED, progress=False)

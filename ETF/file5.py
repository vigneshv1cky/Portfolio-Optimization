...
if not df.empty:
    ...
    cols[0].metric(
        ...
        (
            shorten_name(df.loc[df[ranking_choice].idxmax(), "Sector"])
            if not df[ranking_choice].isnull().all()
            else "N/A"
        ),
    ...
    cols[1].metric(
        ...
        (
            shorten_name(df.loc[df[ranking_choice].idxmin(), "Sector"])
            if not df[ranking_choice].isnull().all()
            else "N/A"
        ),
...
with tab1:
    ...
    if not df.empty:
        ...
        st.write(
            "Vol (20D, ann) is 20-day annualized volatility. It measures how much a sector’s or "
            "stock’s price fluctuates (its risk) based on the last 20 trading days (about one month)."
        )
    else:
...
with tab2:
    ...
    if not df.empty:
        ...
        if not stocks:
            ...
            st.write(
                f"Fetching 1-year data for **{len(stocks)}** stocks — taking the **top 10 stocks** from "
                "each of the **top performing sectors**."
            )
            ...
            if not stocks:
                ...
                try:
                    ...
                    if len(stock_data) < MIN_DATA_POINTS:
                        st.warning(
                            f"Could not retrieve sufficient price data for {selected_stock}."
                            "Found {len(stock_data)} data points, but need at least {MIN_DATA_POINTS} "
                            "for the 100-day moving average to plot."
                        )
                    else:
